...
def writeInFinalFile(node, url, totalLoc, totalSloc, validFiles, invalidFiles, filesNotRead, filesNoMetric, cc, ccRank,
    ...
    final.writerow((node[0], url, str(node[3]), node[6], str(totalLoc), str(totalSloc), str(validFiles),
                    str(invalidFiles), str(filesNotRead), str(filesNoMetric), str(cc), ccRank, str(miMultiFalse),
                    miMultiFalseRank, str(miMultiTrue), miMultiTrueRank, str(difficulty), str(effort), str(timeHas),
                    str(bugs)))
    fileFinal.close()


def writeInFinalTagFile(fileNamePath, node, totalLoc, totalSloc, validFiles, invalidFiles, filesNotRead, filesNoMetric,
                        cc, ccRank, miMultiFalse, miMultiFalseRank, miMultiTrue, miMultiTrueRank, difficulty, effort,
                        timeHas, bugs, ):
    fileFinalTag = open(fileNamePath, 'a', newline='')
    finalTag = csv.writer(fileFinalTag)
    finalTag.writerow((node[0], node[1], node[2], node[3], str(totalLoc), str(totalSloc), str(validFiles),
                       str(invalidFiles), str(filesNotRead), str(filesNoMetric), str(cc), ccRank, str(
        miMultiFalse), miMultiFalseRank, str(miMultiTrue), miMultiTrueRank, str(difficulty), str(effort), str(timeHas),
                       str(bugs)))
    fileFinalTag.close()
...
def main():
    ...
    if os.path.exists("baseFiltrada.csv") and not os.path.exists("finalFiltrado.csv"):
        ...
        if not os.path.exists("final.csv"):
            ...
            final = csv.writer(fileFinal)
            final.writerow(('nameWithOwner', 'url', 'releases', 'updateAt', 'totalLoc', 'totalSloc', 'validFiles',
        ...
        for node in base:
            ...
            if ((contNode >= lastLine) and contNode != 0):
                ...
                gitURL = node[1] + ".git"
                t = threadTemp.thread_with_trace(target=calculateMetrics1.cloneAndReadFileAndGetMetrics,
                                                 args=(gitURL, None))
                t.daemon = True
                t.start()
                ...
                if t.is_alive():  # If thread is still active
                    ...
                    if int(m['cc']) != -1:
                        ...
                        print("--------------")
                        print("Gravando: ")
                        print("Total loc: " + str(m['totalLoc']) + " - Files validos: " + str(
                            m['validFiles']) + " - Files invalidos: " + str(
                            m['invalidFiles']) + "\nFiles sem metricas: " + str(
                            m['filesNoMetric']) + " - Files não lidos: " + str(m['filesNotRead']))
                        print("cc: " + str(m['cc']) + " - miMultiFalse: " + str(
                            m['miMultiFalse']) + "\ndifficulty: " + str(m['difficulty']) + " - bugs: " + str(m['bugs']))
                        writeInFinalFile(node, gitURL, m['totalLoc'], m['totalSloc'], m['validFiles'],
                                         m['invalidFiles'], m['filesNotRead'], m['filesNoMetric'], m['cc'], ccRank,
                                         m['miMultiFalse'], miMultiFalseRank, m['miMultiTrue'], miMultiTrueRank,
                                         m['difficulty'], m['effort'], m['timeHas'], m['bugs'])
                    else:
                        writeInFinalFile(node, gitURL, -1, -1, -1, -1, -1, -1, -1, "Z", -1, "Z", -1, "Z", -1, -1, -1,
                                         -1)
            contNode += 1
        fileBase.close()
        print("\n ------------------- Fim arquivo final ------------------- \n")
            ...
            if not os.path.isfile(fileNamePath):
                ...
                for node in fileRepoTag:
                    if contNode < 2:
                        ...
                        time.sleep(3)
                        t1 = threadTemp.thread_with_trace(target=calculateMetrics2.cloneAndReadFileAndGetMetrics,
                                                          args=(gitURL, tagName))
                        t1.daemon = True
                        t1.start()
                        ...
                        if t1.is_alive():  # If thread is still active
                            ...
                            writeInFinalTagFile(fileNamePath, node, -1, -1, -1, -1, -1, -1, -1, "Z", -1, "Z", -1, "Z",
                                                -1, -1, -1, -1)
                        else:
                            ...
                            if int(m['cc']) != -1:
                                ...
                                print("Total loc: " + str(m['totalLoc']) + " - Files validos: " + str(
                                    ...
                                    m['filesNoMetric']) + " - Files não lidos: " + str(m['filesNotRead']))
                                print("cc: " + str(m['cc']) + " - miMultiFalse: " + str(
                                    m['miMultiFalse']) + "\ndifficulty: " + str(m['difficulty']) + " - bugs: " + str(
                                    m['bugs']))
                                writeInFinalTagFile(fileNamePath, node, m['totalLoc'], m['totalSloc'], m['validFiles'],
                                                    m['invalidFiles'], m['filesNotRead'], m['filesNoMetric'], m['cc'],
                                                    ccRank, m['miMultiFalse'], miMultiFalseRank, m['miMultiTrue'],
                                                    miMultiTrueRank, m['difficulty'], m['effort'], m['timeHas'],
                                                    m['bugs'])
                            else:
                                writeInFinalTagFile(fileNamePath, node, -1, -1, -1, -1, -1, -1, -1, "Z", -1, "Z", -1,
                                                    "Z", -1, -1, -1, -1)
                    contNode += 1
